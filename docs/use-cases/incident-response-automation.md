...
- Use [Tool Filtering](../configuration/tool-filtering.md) with a focused allow list for response workflows — e.g. only expose `create_incident`, `manage_incidents`, `add_responders`, `add_note_to_incident` to prevent the AI from touching unrelated resources

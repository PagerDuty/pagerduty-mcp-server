...
class TestIncidentTools(unittest.TestCase, ContextTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.sample_user_data = Mock(User)
    ...
    def setUp(self):
        ...
        self.mock_user = self.sample_user_data
    ...
    def test_list_incidents_basic(self, mock_paginate):
        ...
        query = IncidentQuery()
        result = list_incidents(query, ctx=self.mock_context)
    ...
    def test_list_incidents_assigned_scope(self, mock_paginate):
        ...
        _ = list_incidents(query, ctx=self.mock_context)
    ...
    def test_list_incidents_user_required_error(self):
        ...
        mock_context = self.create_mock_context(client=self.mock_client, user=None)

        # Test with user required query
        query = IncidentQuery(request_scope="assigned")

        with self.assertRaises(Exception) as context:
            list_incidents(query, ctx=mock_context)

        self.assertIn("A user is required for the request scope.", str(context.exception))
    ...
    def test_get_incident_success(self):
        ...
        # Setup mock
        self.mock_client.rget.return_value = self.sample_incident_data

        # Test
        result = get_incident("PINCIDENT123", ctx=self.mock_context)

        # Assertions
        self.assertIsInstance(result, Incident)
        self.assertEqual(result.id, "PINCIDENT123")
        self.mock_client.rget.assert_called_once_with("/incidents/PINCIDENT123")

    def test_get_incident_api_error(self):
        ...
        with self.assertRaises(Exception) as context:
            get_incident("PINCIDENT123", ctx=self.mock_context)
    ...
    def test_create_incident_success(self):
        ...
        # Setup mock
        self.mock_client.rpost.return_value = self.sample_incident_data
        ...
        # Test
        result = create_incident(create_request, ctx=self.mock_context)
    ...
    def test_change_incident_status(self):
        """Test _change_incident_status helper function."""
        # Setup mock
        self.mock_client.rput.return_value = [self.sample_incident_data]

        # Test
        _change_incident_status(["PINC1"], "acknowledged", client=self.mock_client)
    ...
    def test_change_incident_urgency(self):
        """Test _change_incident_urgency helper function."""
        # Setup mock
        self.mock_client.rput.return_value = [self.sample_incident_data]

        # Test
        _change_incident_urgency(["PINC1"], "low", client=self.mock_client)
    ...
    def test_reassign_incident(self, mock_datetime):
        ...
        _reassign_incident(["PINC1"], assignee, client=self.mock_client)

        # Verify the request structure
        call_args = self.mock_client.rput.call_args
    ...
    def test_escalate_incident(self):
        ...
        # Test
        _escalate_incident(["PINC1"], 2, client=self.mock_client)
    ...
    def test_manage_incidents_status_change(self, mock_change_status):
        ...
        result = manage_incidents(manage_request, ctx=self.mock_context)

        # Assertions
        self.assertIsInstance(result, ListResponseModel)
        self.assertEqual(len(result.response), 1)
        mock_change_status.assert_called_once_with(["PINC1"], "acknowledged", self.mock_client)
    ...
    def test_manage_incidents_urgency_change(self, mock_change_urgency):
        ...
        self.assertIsInstance(result, ListResponseModel)
        self.assertEqual(len(result.response), 1)
        mock_change_urgency.assert_called_once_with(["PINC1"], "low", self.mock_client)
    ...
    def test_manage_incidents_reassignment(self, mock_reassign):
        ...
        self.assertIsInstance(result, ListResponseModel)
        self.assertEqual(len(result.response), 1)
        mock_reassign.assert_called_once_with(["PINC1"], assignee, self.mock_client)
    ...
    def test_manage_incidents_escalation(self, mock_escalate):
        ...
        self.assertIsInstance(result, ListResponseModel)
        self.assertEqual(len(result.response), 1)
        mock_escalate.assert_called_once_with(["PINC1"], 2, self.mock_client)
    ...
    def test_add_responders_success(self):
        ...
        result = add_responders("PINC1", request, ctx=self.mock_context)
    ...
    def test_add_responders_no_user_context(self):
        ...
        # Test
        request = IncidentResponderRequest(requester_id="PUSER123", message="Help needed", responder_request_targets=[])
        result = add_responders("PINC1", request, ctx=mock_context)
    ...
    def test_add_responders_unexpected_response(self):
        ...
        self.mock_client.rpost.return_value = "Unexpected response"
        self.mock_user = Mock()
        self.mock_user.id = "PUSER123"
    ...
    def test_add_responders_mixed_targets_payload(self):
        ...
        self.mock_client.rpost.return_value = {
            "responder_request": {
                "requester": {"id": "PUSER123", "type": "user_reference"},
                "message": "Help needed",
                "requested_at": "2023-01-01T00:00:00Z",
        ...
        # Execute
        _ = add_responders("PINC1", request, ctx=self.mock_context)
    ...
    def test_add_note_to_incident_success(self):
        ...
        # Test
        result = add_note_to_incident("PINC123", "This is a test note", ctx=self.mock_context)
    ...
    def test_list_incident_notes_success(self):
        ...
        self.mock_client.rget.return_value = mock_response

        # Test
        result = list_incident_notes("PINC123", ctx=self.mock_context)
        ...
        # Verify API call
        self.mock_client.rget.assert_called_once_with("/incidents/PINC123/notes")

    def test_list_incident_notes_empty(self):
    ...
    def test_get_outlier_incident_success(self):
        """Test getting outlier incident successfully."""
        # Setup mock
        self.mock_client.rget.return_value = self.sample_outlier_incident_data

        # Test
        query = OutlierIncidentQuery()
        result = get_outlier_incident("PINCIDENT123", query, ctx=self.mock_context)
        ...
        self.mock_client.rget.assert_called_once_with("/incidents/PINCIDENT123/outlier_incident", params={})

    def test_get_outlier_incident_with_params(self):
    ...
    def test_get_outlier_incident_without_title(self):
        ...
        self.mock_client.rget.return_value = outlier_data_no_title
    ...
    def test_get_past_incidents_success(self):
        """Test getting past incidents successfully."""
        # Setup mock
        self.mock_client.rget.return_value = self.sample_past_incidents_data

        # Test
        query = PastIncidentsQuery()
        result = get_past_incidents("PINCIDENT123", query, ctx=self.mock_context)
        ...
        self.mock_client.rget.assert_called_once_with("/incidents/PINCIDENT123/past_incidents", params={})
    ...
    def test_get_related_incidents_success(self):
        """Test getting related incidents successfully."""
        # Setup mock
        self.mock_client.rget.return_value = self.sample_related_incidents_data

        # Test
        query = RelatedIncidentsQuery()
        result = get_related_incidents("PINCIDENT123", query, ctx=self.mock_context)
        ...
        self.mock_client.rget.assert_called_once_with("/incidents/PINCIDENT123/related_incidents", params={})

    def test_get_related_incidents_with_params(self):
    ...
    def test_get_outlier_incident_api_error(self):
        ...
        query = OutlierIncidentQuery()
        with self.assertRaises(Exception) as context:
            get_outlier_incident("PINCIDENT123", query, ctx=self.mock_context)

        self.assertIn("API Error", str(context.exception))

    def test_get_past_incidents_api_error(self):
        ...
        query = PastIncidentsQuery()
        with self.assertRaises(Exception) as context:
            get_past_incidents("PINCIDENT123", query, ctx=self.mock_context)

        self.assertIn("API Error", str(context.exception))

    def test_get_related_incidents_api_error(self):
        ...
        query = RelatedIncidentsQuery()
        with self.assertRaises(Exception) as context:
            get_related_incidents("PINCIDENT123", query, ctx=self.mock_context)

        self.assertIn("API Error", str(context.exception))

    def test_get_related_incidents_empty_list_response(self):
    ...
    def test_get_outlier_incident_direct_response(self):
        ...
        self.mock_client.rget.return_value = direct_response
    ...
    def test_get_past_incidents_empty_list_response(self):
...
class TestAlertTools(unittest.TestCase, ContextTestCase):
    ...
    def setUp(self):
        self.mock_client = self.create_mock_client()
        self.mock_context = self.create_mock_context(self.mock_client)

    def test_get_alert_from_incident(self):
        """Test getting a specific alert from an incident."""
        # Arrange
        self.mock_client.rget.return_value = self.sample_alert_data

        # Act
        result = get_alert_from_incident("PINCIDENT123", "PALERT123", ctx=self.mock_context)
        ...
        self.mock_client.rget.assert_called_once_with("/incidents/PINCIDENT123/alerts/PALERT123")
    ...
    def test_list_alerts_from_incident(self, mock_paginate):
        ...
        # Act
        result = list_alerts_from_incident("PINCIDENT123", query_model, ctx=self.mock_context)

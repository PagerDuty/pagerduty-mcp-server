...
The PagerDuty MCP Server exposes **84 tools** across **18 domains**. By default, only the 55 read-only tools are available. The 29 write tools require starting the server with `--enable-write-tools`.

## Read vs. Write Tools

| Type | Count | Flag Required |
|------|-------|---------------|
| Read-only | 55 | None (always available) |
| Write | 29 | `--enable-write-tools` |
| **Total** | **84** | |
...
| `create_incident_status_update` | Write | Post a status update to an incident for subscribers |
...
| `list_schedule_overrides` | Read | List overrides for a schedule in a date range |
...
| Tool | Type | Description |
|------|------|-------------|
| `get_oncall_compensation_report` | Read | Generate a detailed on-call compensation and burden report |
...
- [Schedules](./schedules) · [On-Call](./oncalls) · [On-Call Compensation](./oncall-compensation)
- [Priorities](./priorities) · [Log Entries](./log-entries)

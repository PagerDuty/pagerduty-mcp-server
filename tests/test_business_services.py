...
from pagerduty_mcp.tools.services import get_technical_service_dependencies
...
class TestTechnicalServiceDependencies(unittest.TestCase):
    ...
    @patch("pagerduty_mcp.tools.services.get_client")
    def test_get_technical_service_dependencies_calls_get_correctly(self, mock_get_client):
        mock_get_client.return_value = self.mock_client
        mock_response = MagicMock()
        mock_response.json.return_value = {"relationships": []}
        self.mock_client.get.return_value = mock_response
        ...
        self.mock_client.get.assert_called_once_with(
            "/service_dependencies/technical_services/SVC123"
        )

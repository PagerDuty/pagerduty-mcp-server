from pagerduty_mcp.models import EscalationPolicy, EscalationPolicyQuery, ListResponseModel, MCPContext
from pagerduty_mcp.utils import inject_context, paginate


@inject_context
def list_escalation_policies(
    query_model: EscalationPolicyQuery, context: MCPContext
) -> ListResponseModel[EscalationPolicy]:
    """List escalation policies with optional filtering.

    Args:
        query_model: Optional filtering parameters
        context: The MCP context containing the PagerDuty REST API client (injected)

    Returns:
        List of escalation policies matching the query parameters
    """
    response = paginate(client=context.client, entity="escalation_policies", params=query_model.to_params())
    policies = [EscalationPolicy(**policy) for policy in response]
    return ListResponseModel[EscalationPolicy](response=policies)


@inject_context
def get_escalation_policy(policy_id: str, context: MCPContext) -> EscalationPolicy:
    """Get a specific escalation policy.

    Args:
        policy_id: The ID of the escalation policy to retrieve
        context: The MCP context containing the PagerDuty REST API client (injected)

    Returns:
        Escalation policy details
    """
    response = context.client.rget(f"/escalation_policies/{policy_id}")
    return EscalationPolicy.model_validate(response)

...
- **Tool filtering** via [`mcp-proxy`](../configuration/tool-filtering.md) is not directly available for the remote server (the proxy approach requires a local subprocess to intercept), but you can limit which tools will succeed with a [Scoped OAuth client][scopes].
